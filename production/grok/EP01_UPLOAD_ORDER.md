...
### SH001 · Панель лифта · генерируем 10 сек, в монтаж 2 сек
...
### SH002 · Зеркало в кабине · генерируем 10 сек, в монтаж 2 сек
...
### SH003 · Палец не касается · генерируем 8 сек, в монтаж 3 сек
...
### SH004 · Мигание трубки · генерируем 8 сек, в монтаж 3 сек
...
### SH005 · Четыре шага · генерируем 8 сек, в монтаж 3 сек
...
### SH008 · Узнавание · генерируем 8 сек, в монтаж 3 сек
...
### SH012 · Щель. Утро · генерируем 8 сек, в монтаж 3 сек
...
## Поправка по длительности — 21.08.2026

Grok генерирует до 15 секунд. Пятнадцать секунд одной генерацией серию не
собирают: между нашими шотами жёсткие склейки, а модель выдаёт непрерывное
движение — вместо смены кадра получится пролёт, и ритм 1,5–3 секунды на шот,
на котором держится удержание, исчезнет.

**Но запас длительности берём иначе.** Чем длиннее дубль, тем выше шанс, что
нужное событие произойдёт правильно хотя бы раз внутри него. Режем мы всё равно
до двух-трёх секунд, значит можем выбирать лучший кусок, а не первый попавшийся.

Поднято там, где событие сложное или его легко промазать:

| Шот | Генерируем | В монтаж | Причина |
|---|--:|--:|---|
| SH002 | 10 | 2,0 | задержка отражения — самый тонкий приём серии |
| SH003 | 8 | 3,0 | три события подряд: вход, остановка, угасание |
| SH005 | 8 | 3,0 | проход занимает время, нужен запас на шаги |
| SH008 | 8 | 3,0 | держим без склейки, выбираем лучший вдох |
| SH012 | 8 | 3,0 | тень должна пересечь полосу целиком |

Остальные девять остаются короткими: одно движение на одном предмете,
лишние секунды дают только дрейф.

## Почему не грузим много референсов за раз

Референсы не складываются, а конкурируют. Проверено 21.08.2026: одна лишняя
картинка на `soul_2` переписала промпт и подменила персонажа — другое лицо,
борода, синие глаза. Сработал минимальный сильный набор.

Отдельный риск: turnaround в шоте, где лица нет (SH006, SH007, SH009, SH014),
заставит модель искать, куда поместить человека. В макро с ручкой это выйдет
рукой или отражением лица в латуни.

**Правило.** Стартовый кадр всегда первым. Дальше максимум один уточняющий:
конечное состояние, лицо или локация. Больше двух ни в одном шоте не нужно.
