...
Эталоны наследуются не пересказом, а самим кадром: принятое задание передаётся
в `medias` с ролью `image_references`. У `nano_banana_pro` **нет параметра
`enhance_prompt`** — той ловушки, которая в прошлый раз переписала промпт и
уничтожила идентичность на `soul_2`. Поэтому референс-кондишенинг здесь безопасен,
и запрет из `VIDEO_ENGINES.md` §3-бис сужается: он касается только `soul_2`.

| Что генерим | Референс |
|---|---|
| кадр с лицом Прохора | turnaround `1ac61c5a-517e-448f-885c-061d2c8daa85` |
| кадр в коридоре | коридор `7c9c1518-7f6b-49d0-9d89-29b78805c5cd` |
| кадр у двери 46 | дверь `07543c3f-014d-4b4d-917d-81db7fb9e57f` |
| кадр с ручкой | ручка `e79996c1-5d64-48cc-a059-2659e9a57cd2` |

### Проверочная пара — 21.08.2026

Два кадра запущены раньше остальных, чтобы проверить оба канала наследования
до того, как в них вложатся двенадцать генераций.

| Шот | Что проверяем | Job ID |
|---|---|---|
| sh02 | переносится ли лицо с turnaround | `205b6ccd-6f86-405c-a753-211e0c8422c4` |
| sh04 | переносится ли локация с эталона коридора | `daedc142-05dc-4388-8a1b-eb57c98ae836` |

Проверка №0 пройдена: промпты вернулись дословно, `input_images` в обоих
заданиях содержит нужный эталон.


### Разбор проверочной пары — 21.08.2026

**sh04 — ПРИНЯТ.** Локация перенеслась полностью: те же стены, дорожка, дверь
в торце, двери вдоль стен, узкая молочная полоса, воздух сверху под субтитры.
Канал «эталон локации → кадр» работает. Кадр становится обложкой сериала.

**sh02 — отклонён, но канал «эталон лица → кадр» работает.** Архитектура лица
перенеслась: узкий вытянутый овал, высокий лоб, тяжёлые веки, холодные серо-голубые
глаза, трёхдневная щетина, поры без ретуши. Брак в четырёх местах:

| Что | Диагноз |
|---|---|
| шрам на лбу и слева | канон — тонкий рубец **через правую бровь**, первый пункт чек-листа §7 |
| отражение показывает спину | зеркало не может показать затылок человека, стоящего к нему лицом; читается как второй человек в кабине |
| нашивка на спине — в отражении | правило «нашивки нет» не было сказано про отражение |
| лицо освещено ровно | по библии правая половина — чистый чёрный без заполнения |

Три из четырёх — недосказанность промпта, не сбой модели. Исправления внесены
утверждениями: бровь названа через сторону кадра, лоб объявлен целым, зеркало
описано как держащее профиль, лампа помещена слева от героя.

### Батч мастер-кадров — 21.08.2026

Восемь кадров: один переснятый и семь новых, наследующих принятые эталоны.

| Шот | Наследует | Job ID |
|---|---|---|
| sh01 панель лифта | — (задаёт канон панели) | `8aee8528-24da-464b-987a-12caec9ccc1d` |
| sh02 зеркало, v2 | turnaround | `c6ee22d8-3f84-4aeb-bb8a-befcfc992b84` |
| sh05 спина в коридоре | коридор | `19a661c6-80ff-4c48-9270-b4bb221dd1cd` |
| sh06 брелок | ручка | `839ab35b-2377-4c22-b305-54b4ce069903` |
| sh07 волос | ручка | `c4d82186-5cc7-4ae6-bbd2-f475a196d9c6` |
| sh09 ссадина | ручка | `3992b482-0744-42d4-8767-1f300078c386` |
| sh12 щель / утро | — | `6e978c5b-5616-4a43-a766-755575ccd28b` |
| sh14 резинка | ручка | `5938adbe-69ac-4406-aace-eba006b39756` |

**Кнопки лифта нарисованы пустыми намеренно.** Цифры — текст, а текст рисуется
в композе: `TYPOGRAPHY.md` §5-бис, исключений нет. То же с номером «46» на двери
в sh06 и с интерфейсом телефона в sh13.

Ждут своей очереди: sh03 (нужен принятый sh01), sh08 и sh11 (нужен принятый sh02),
sh10 и sh13 (собираются из двух генераций с маской).


### Разбор первого батча — 21.08.2026

**Принято три кадра:**

| Шот | Job ID | Чем взял |
|---|---|---|
| sh05 спина в коридоре | `19a661c6-80ff-4c48-9270-b4bb221dd1cd` | локация совпала с эталоном, лица нет, спина чистая, контровой по плечам |
| sh07 волос | `c4d82186-5cc7-4ae6-bbd2-f475a196d9c6` | один волос, не прядь, с изломом, светится ниткой на тёмной латуни |
| sh04 коридор | `daedc142-05dc-4388-8a1b-eb57c98ae836` | принят раньше, становится обложкой |

**Отклонено пять. Общая причина у трёх — свет.**

`nano_banana_pro` по умолчанию снимает макро как предметную съёмку: высокий ключ,
чистая белизна, ровное заполнение. Кадры sh06, sh09 и sh14 пришли технически
безупречными и абсолютно не ночными — рядом с коридором они выпадали из серии.

**Правило света для макро, обязательное с этого момента.** Источник называется
единственным и помещается в кадр словами, а темнота задаётся утверждением о том,
куда свет не доходит:

```
The only light is a narrow warm slit escaping from under the door low in the frame,
grazing upward across the ...; above it the surface falls away into deep shadow
and the top of the frame is nearly black.
```
...
| Шот | Диагноз |
|---|---|
| sh01 панель | кнопок шесть вместо девяти, а вспучина стоит **над** колонкой; по сценарию металл продавливается **под** девяткой |
| sh02 зеркало | шрам и зеркало исправлены, но лицо освещено ровно; правило половины в чистом чёрном не выполнено |
| sh09 ссадина | нет самого брелока: в промпте было красное колечко и не было рыбки — недоработка промптера |
| sh12 щель | **лицо ребёнка в кадре** — прямое нарушение `characters/adelaida/IDENTITY_LOCK.md`; плюс щель распахнута, а по канону дверь открывается ровно на 2 см |

**sh12 — самая опасная ошибка батча.** Кадр красивый, и именно поэтому опасный:
ребёнок в этом сериале существует силуэтом, затылком, рукой, пижамой, пустой
кроватью и голосом. Показанное лицо обнуляет приём на весь сезон. В переснятом
варианте ребёнок присутствует только тенью, идущей по полу и скатерти.

| Шот | Пересъёмка |
|---|---|
| sh01 | `b1fcec11-fc39-4905-a393-b3f80050a6ad` |
| sh02 | `4752cbe1-68c6-49f8-b6a8-3cb9563c5478` |
| sh06 | `8c77e469-2429-46b7-8d89-1f4afd53dec5` |
| sh09 | `7d3af4de-390c-4bdb-909e-967b62d202e2` |
| sh12 | `4a0182f5-0c45-4cd4-a4ec-668d760e4b31` |
| sh14 | `0f6c9df1-1bdf-4ba4-8561-a477c5e74123` |


### Два отказа по драматургии, а не по качеству — 21.08.2026

Кадры sh10 и sh13 пришли технически чистыми и были отклонены за то,
что приём в них поставлен не туда, где он работает.

**sh10.** Латунная табличка встала на переднем плане у лифта вместо глубины коридора.
Весь шот построен на том, что зритель смотрит на Прохора, а буквы гаснут у него
за спиной, во втором плане, куда камера намеренно не помогает смотреть
(`STORY_RULES`: цена показывается там, куда зритель не смотрит). Табличка под носом
делает гаснущие буквы главным событием кадра и убивает приём.

**sh13.** Край белой ткани в проёме детской вышел крупным и заметным с первого раза.
Это `REWATCH DETAIL`, посеянный на EP07: его замечают при пересмотре. Деталь,
видимая сходу, отменяет саму механику перепросмотра — пересматривать становится
нечего. Нужен край в углу, который проще пропустить, чем увидеть.

**Правило, которое из этого следует.** У кадра есть не только содержание, но и
**место приёма в композиции**. Проверка перед приёмкой: не «есть ли деталь в кадре»,
а «стоит ли она там, где делает свою работу».

| Тип детали | Где обязана стоять |
|---|---|
| `WRONG DETAIL` | в зоне внимания, но не в её центре |
| `VISUAL REVEAL` | во втором плане, куда камера не доводит взгляд |
| `REWATCH DETAIL` | на краю кадра, мельче порога первого просмотра |
| `EXTREME DETAIL` | один объект на весь кадр, фон уходит в расфокус |

| Шот | Пересъёмка |
|---|---|
| sh10 | `bd4e088d-e13e-4dff-be9e-14f9df8f5a68` |
| sh13 | `d534ed8f-bebf-442d-bb6c-8dfdc631aff5` |


### sh10, попытка вторая — отказ, ошибка переехала

Табличка ушла с переднего плана и встала на **дальнюю дверь**, в геометрический
центр кадра. Два нарушения сразу:

1. Дальняя дверь — это дверь 46. У сорок шестой таблички нет и быть не может:
   её отсутствие — часть того, почему квартира неправильная. Правило внесено
   в `bible/CONTINUITY.md` §8.
2. Конец тоннеля в одноточечной перспективе — самая рассматриваемая точка кадра.
   `VISUAL REVEAL` требует обратного: приём стоит там, куда камера не доводит взгляд.

**Урок промптинга.** Убрать деталь из неверного места мало — модель поставит её
в следующее по очевидности. Место приёма задаётся адресно: не «в глубине»,
а «на второй двери левой стены, в глубине, левее центра». А место, куда деталь
попасть не должна, закрывается утверждением о том, что там пусто:
`the door at the far end is plain painted wood, its whole surface empty and unbroken`.

Третья попытка: `268becae-893c-4a8a-8b21-b20b1efc4151`.


## 7. Мастер-кадры закрыты — 21.08.2026

Четырнадцать кадров из четырнадцати. Сводка по всему этапу:

| Метрика | Значение |
|---|---|
| Кадров в эпизоде | 14 |
| Всего генераций | 31 |
| Принято с первой попытки | 6 |
| Средняя цена кадра | 2,2 генерации |
| Движок | `nano_banana_pro` — все 14 |
| Отказов по свету | 3 |
| Отказов по канону мира | 4 |
| Отказов по месту приёма в кадре | 3 |
| Отказов по анатомии и геометрии предмета | 1 |

**Ни одного отказа по идентичности героя.** Referencing через `image_references`
с принятым turnaround отработал во всех кадрах с лицом. Запрет на референс-кондишенинг
из `VIDEO_ENGINES.md` §3-бис касается только `soul_2` с его принудительным
`enhance_prompt`.

**Ни одного отказа по мусорному тексту и плёночной разметке** — после того,
как негативные списки были опустошены целиком.

### Что вошло в метод за этот этап

1. Модель рисует каждое существительное, где бы оно ни стояло.
2. `film grain` вызывает краевую разметку плёнки.
3. Служебное слово о камере рисуется как предмет.
4. Макро по умолчанию снимается как предметная съёмка: ночь задаётся единственным
   названным источником и утверждением о том, куда свет не доходит.
5. У приёма есть место в композиции, и оно задаётся адресно.
6. Убрать деталь из неверного места мало — модель поставит её в следующее
   по очевидности. Пустое место закрывается утверждением о пустоте.
7. Идентичность, которая не воспроизводится устойчиво, переводится в композ.
