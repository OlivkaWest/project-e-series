...
# ── 1. тело 0,00-14,90: MAIN ложится один в один ────────────────────────────
# у трека собственное нарастание: тихо до 5,0; громко 6,0-8,5 (мотив под
# коридором); откат 9-13; подъём с 13,5 под DANGER. Совпадает с монтажом.
BODY_END, MUTE_IN = 14.70, 0.20
seg = main[: int((BODY_END + MUTE_IN) * SR)]
place(tr, ramp(seg, fin=0.05, fout=MUTE_IN), 0.0, 1.0)

# ── 2. DANGER подкладывается с 10,40, пик приходится на 13,90 ───────────────
# у трека нарастание 0->3,5 с пиком -11 дБ. Ставим так, чтобы пик пришёлся
# на SH011 (свет по глазам), а начало подъёма — на SH006 (брелок).
D_AT = 10.40
d_len = BODY_END + MUTE_IN - D_AT
place(tr, ramp(danger[: int(d_len * SR)], fin=0.35, fout=MUTE_IN), D_AT, 0.72)

# ── 3. тишина 14,90-16,30: все слои сняты (правило 4) ───────────────────────
SIL_A, SIL_B = 14.90, 16.30
